...
class Mysql(BaseDb):
	...
	def get_connection(self):
		if self.schema.connection.connection_type.name == 'ODBC':
			return super(Mysql, self).get_connection()
	...
	def insert_temporary_data(self, temporary_table, table_name):
		...
		cursor = self.query("SELECT * FROM {0};".format(temporary_table))

		if not cursor:
			return False
		
		result = self.insert_from_cursor(table_name, cursor, fetch_rows_qty)
		cursor.close()
		return result
